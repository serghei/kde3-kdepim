    ...
    if(WebdavHandler::extractLong(node, "sensitivity", tmplng))
    {
        switch(tmplng)
        {
            ...
            case 3:
                ...
                break;
            default:
                kdWarning() << "Unknown sensitivity: " << tmplng << endl;
        }
    }
...
protected:
    void addBoolProp(QDomElement &el)
    {
        el.setAttribute("b:dt", "boolean");
    }
    void addDateProp(QDomElement &el)
    {
        el.setAttribute("b:dt", "dateTime.tz");
    }
    void addFloatProp(QDomElement &el)
    {
        el.setAttribute("b:dt", "float");
    }
    void addIntProp(QDomElement &el)
    {
        el.setAttribute("b:dt", "int");
    }
    QString timePropString(const QDateTime &dt)
    {
        return dt.toString(Qt::ISODate) + "Z";
    }
    ...
    bool visitIncidence(Incidence *incidence)
    {
        ...
        {
            case KCal::Incidence::SecrecyPublic:
                tmpstr = "0";
                break;
            case KCal::Incidence::SecrecyPrivate:
                tmpstr = "2";
                break;
            case KCal::Incidence::SecrecyConfidential:
                tmpstr = "3";
                break;
            default:
                break;
    ...
    bool visit(Event *event)
    {
        ...
        {
            case KCal::Event::Transparent:
                tmpstr = "FREE";
                break;
            case KCal::Event::Opaque:
                tmpstr = "BUSY";
                break;
        }
        if(!tmpstr.isEmpty())
