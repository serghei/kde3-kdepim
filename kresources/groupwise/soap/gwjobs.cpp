...
void ReadAddressBooksJob::readAddressBook(std::string &id)
{
    ...
    else
    {
        ...
        {
            kdDebug() << "  Couldn't read " << GWConverter::stringToQString(id) << " : " << GWConverter::stringToQString(
                          cursorResponse.status->description) << endl;
    ...
    *(readCursorRequest.count) = (int)readChunkSize;
    ...
    while(true)
    {
        ...
        if(readCursorResponse.items)
        {
            ...
            kdDebug() << "ReadAddressBooksJob::readAddressBook() - got " << readCursorResponse.items->item.size() << " contacts in cursor read of folder " <<
                      id.c_str()   << endl;
            ...
            readItems +=
                readCursorResponse.items->item.size(); // this means that the read count is increased even if the call fails, but at least the while will always end
...
void ReadCalendarJob::run()
{
            ...
            for(it = folders->begin(); it != folders->end(); ++it)
            {
                ...
                else
                {
                    ...
                    if(fld)
                    {
                        ...
                        if(haveReadFolder)
                        {
                            kdDebug() << "Folder contained " << itemCounts.appointments << " appointments, " << itemCounts.notes << " notes, and " << itemCounts.tasks <<
                                      " tasks." << endl;
    ...
    kdDebug() << "Folders we read contained " << totals.appointments << " appointments, " << totals.notes << " notes, and " << totals.tasks << " tasks."
              << endl;
    kdDebug() << "Local calendar now contains " << mCalendar->rawEvents().count() << " events and " << mCalendar->rawJournals().count() <<
              " journals, and " << mCalendar->rawTodos().count() << " todos." << endl;
