...
struct compressed_page_traits
{
    ...
    {
        return compressed_page::header_size;
    }

    static unsigned min_size()
    {
        return 16;    // zlib needs minimum 12 bytes
    }
    ...
    {
        set_compression_manager(m);
    }
    static void print(std::ostream &out, pointer p)
    {
        out << '[' << p << "] compressed_page:\n";
        p->print(out);
    }
};
...
private:
    ...
    static unsigned pageidx(unsigned idx)
    {
        return idx >> page_bits;
    }
    static unsigned inpageidx(unsigned idx)
    {
        return idx & ((1 << page_bits) - 1);
    }
    struct page
    {
