...
leafdata_iterator begin() const
{
    return iterator(const_cast<unsigned char *>(my_base()));
}
leafdata_iterator end() const
{
    return iterator(const_cast<unsigned char *>(my_base()) + usedbytes());
}
...
static void init(pointer<leaf_data> p)
{
    construct(p.raw_pointer());
}
static unsigned start_bytes()
{
    return 16;
}
END_THING(leaf_data)
...
struct leaf_data_pool_traits
{
    ...
    static bool is_free(pointer p)
    {
        return p->capacity() == 0;
    }
    static void mark_free(pointer p)
    {
        memset(p.raw_pointer(), 0, p->capacity());
    }
    static unsigned size_of(pointer p)
    {
        return p->capacity() + leafdata::data_offset;
    }

    static unsigned type_offset()
    {
        return 2;
    }
    ...
    static void set_manager(memory_manager *m)
    {
        return set_leafdata_manager(m);
    }
    static void print(std::ostream &out, pointer p)
    {
        out << '[' << p << "] leafdata:\n";
        p->print(out);
    }
