...
Kontact::ProfileDialog::ProfileDialog(QWidget *parent,
                                      WFlags flags) : KDialogBase(parent, /*name=*/0, /*modal=*/true, /*caption=*/QString(), /*buttonMask=*/KDialogBase::Ok | KDialogBase::Close)
{
    setWFlags(flags);
...
void Kontact::ProfileDialog::profileLoaded(const QString &id)
{
    ...
    if(profile.isNull())
        return;
    KMessageBox::information(this, i18n("The profile \"%1\" was successfully loaded. Some profile settings require a restart to get activated.").arg(
                                 profile.name()), i18n("Profile Loaded"));
}
...
void Kontact::ProfileDialog::saveToSelectedProfile()
{
    ...
    if(KMessageBox::Yes != KMessageBox::warningYesNo(this,
            i18n("The profile \"%1\" will be overwritten with the current settings. Are you sure?").arg(profile.name()), i18n("Save to Profile"),
            KStdGuiItem::overwrite(), KStdGuiItem::cancel()))
        return;
...
void Kontact::ProfileDialog::deleteSelectedProfile()
{
    ...
    if(profile.isNull())
        return;
    if(KMessageBox::Yes != KMessageBox::warningYesNo(this,
            i18n("Do you really want to delete the profile \"%1\"? All profile settings will be lost!").arg(profile.name()), i18n("Delete Profile"),
