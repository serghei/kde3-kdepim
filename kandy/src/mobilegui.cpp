...
void MobileGui::writePhonebookPostProcessing()
{
    ...
    else if(mComingFromReadPhonebook)
    {
        mComingFromReadPhonebook = false;
        QString tmp = "";
...
void MobileGui::formatPBName(QString *name, QString suffix)
{
    ...
    else if(name->length() + theSuffix.length() < mPBNameLength)
        ...
        while(fields)
        {
            for(unsigned int i = 0; i < fields->count(); i++)
            {
                ...
                if(memory == "FD")
                    ...
                    mMobHasOW = true;
                else if((mMobManufacturer == "SIEMENS") && (memory == "MC"))
                    mMobHasMC = true;
                else if((mMobManufacturer == "SIEMENS") && (memory == "RC"))
                    mMobHasRC = true;
            }
