...
public:
    ...
    EmailAddressList(const QValueList<KCal::Person> &list)
    {
        operator=(list);
    }
...
public:
    ...
    {
        return (mActionType == T_MESSAGE || mActionType == T_EMAIL) ? mText : QString::null;
    }
    QString            fileName() const
    {
        return (mActionType == T_FILE) ? mText : QString::null;
    }
    QString            command() const
    {
        return (mActionType == T_COMMAND) ? mText : QString::null;
    }
...
public:
    ...
    Type               type() const
    {
        return static_cast<Type>(mType & ~TIMED_DEFERRAL_FLAG);
    }
    ...
    {
        return (mActionType == T_AUDIO) && !mBeep ? mText : QString::null;
    }
...
public:
    ...
    KAEvent(const QDateTime &dt, const QString &message, const QColor &bg, const QColor &fg, const QFont &f, Action action, int lateCancel, int flags)
        : mRecurrence(0)
    {
        set(dt, message, bg, fg, f, action, lateCancel, flags);
    }
    explicit KAEvent(const KCal::Event &e)  : mRecurrence(0)
    {
        set(e);
    }
    KAEvent(const KAEvent &e)               : KAAlarmEventBase(e), mRecurrence(0)
    {
        copy(e);
    }
    ~KAEvent()
    ...
    void               set(const QDateTime &, const QString &message, const QColor &bg, const QColor &fg, const QFont &, Action, int lateCancel,
                           int flags);
    ...
    {
        mNextMainDateTime.set(d);
        mUpdated = true;
    }
    void               setTime(const QDateTime &dt)
    {
        mNextMainDateTime.set(dt);
        mUpdated = true;
    }
...
private:
    ...
    bool               setRecur(KCal::RecurrenceRule::PeriodType, int freq, int count, const QDateTime &end,
                                KARecurrence::Feb29Type = KARecurrence::FEB29_FEB29);
    void               clearRecur();
