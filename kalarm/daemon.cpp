...
bool Daemon::registerWith(bool reregister)
{
    ...
    kdDebug(5950) << (reregister ? "Daemon::reregisterWith(): " : "Daemon::registerWith(): ") << (disabledIfStopped ? "NO_START" : "COMMAND_LINE") <<
                  endl;
    ...
    if(reregister)
        ...
        s.registerApp(appname, kapp->aboutData()->programName(), QCString(NOTIFY_DCOP_OBJECT), AlarmCalendar::activeCalendar()->urlString(),
                      !disabledIfStopped);
    if(!s.ok())
    {
