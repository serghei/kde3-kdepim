...
void KNMainWidget::setStatusMsg(const QString &text, int id)
{
    ...
    else
    {
        ...
        {
            case SB_MAIN:
                BroadcastStatus::instance()->setStatusMsg(text);
                break;
            case SB_GROUP:
                s_tatusGroup->setText(text);
                break;
            case SB_FILTER:
                s_tatusFilter->setText(text);
                break;
        }
    }
}
...
void KNMainWidget::slotGrpUnsubscribe()
{
    ...
    if(g_rpManager->currentGroup())
    {
        if(KMessageBox::Yes == KMessageBox::questionYesNo(knGlobals.topWidget,
                i18n("Do you really want to unsubscribe from %1?").arg(g_rpManager->currentGroup()->groupname()), QString::null, i18n("Unsubscribe"),
                KStdGuiItem::cancel()))
...
void KNMainWidget::slotFolEmpty()
{
    ...
    if(f_olManager->currentFolder() && !f_olManager->currentFolder()->isRootFolder())
    {
        ...
        if(KMessageBox::Continue == KMessageBox::warningContinueCancel(
                    this, i18n("Do you really want to delete all articles in %1?").arg(f_olManager->currentFolder()->name()), "", KGuiItem(i18n("&Delete"),
                            "editdelete")))
