...
public:
    ...
    {
        if(!create && m_essageID.isEmpty()) return 0;
        return &m_essageID;
    }
    KMime::Headers::From *from(bool create = true)
    {
        if(!create && f_rom.isEmpty()) return 0;
        return &f_rom;
    }
    KMime::Headers::References *references(bool create = true)
    {
        if(!create && r_eferences.isEmpty()) return 0;
        return &r_eferences;
    }
    ...
    {
        return a_rticleNumber;
    }
    void setArticleNumber(int number)
    {
        a_rticleNumber = number;
    }
    ...
    bool isRead()
    {
        return f_lags.get(7) || f_lags.get(3);    // ignored articles == read
    }
    ...
    {
        return s_ubThreadChangeDate;
    }
    void setSubThreadChangeDate(time_t date)
    {
        s_ubThreadChangeDate = date;
    }
...
public:
    ...
    bool pending()
    {
        return ((doPost() && !posted()) || (doMail() && !mailed()));
    }
    bool isSavedRemoteArticle()
    {
        return (!doPost() && !doMail() && editDisabled());
    }
...
public:
    ...
    int cte()
    {
        return e_ncoding.cte();
    }
    void setCte(int e)
    {
        e_ncoding.setCte((KMime::Headers::contentEncoding)(e));
        h_asChanged = true;
    }
    ...
    QString encoding()
    {
        return e_ncoding.asUnicodeString();
    }
