...
public:
    ...
    bool success() const
    {
        return e_rrorString.isEmpty();
    }
    bool canceled() const
    {
        return c_anceled;
    }
    bool authError() const
    {
        return a_uthError;
    }
    ...
    {
        e_rrorString = d_ata->prepareForExecution();
    }
    void notifyConsumer();
    ...
    {
        if(mProgressItem) mProgressItem->setStatus(msg);
    }
    void setProgress(unsigned int progress)
    {
        if(mProgressItem) mProgressItem->setProgress(progress);
    }
    void setComplete()
    {
        if(mProgressItem)
