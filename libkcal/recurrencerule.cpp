...
QDateTime RecurrenceRule::Constraint::intervalDateTime(RecurrenceRule::PeriodType type) const
{
    ...
    switch(type)
    {
        case rSecondly:
            ...
            break;
        case rMinutely:
            dt.setTime(QTime(hour, minute, 1));
            break;
        case rHourly:
            dt.setTime(QTime(hour, 1, 1));
            break;
        case rDaily:
            break;
        case rWeekly:
            dt = DateHelper::getNthWeek(year, weeknumber, weekstart);
            break;
        case rMonthly:
            dt.setDate(QDate(year, month, 1));
            break;
        case rYearly:
...
{
    kdDebug(5800) << "     ~> Y=" << year << ", M=" << month << ", D=" << day << ", H=" << hour << ", m=" << minute << ", S=" << second << ", wd=" <<
                  weekday << ",#wd=" << weekdaynr << ", #w=" << weeknumber << ", yd=" << yearday << endl;
}
