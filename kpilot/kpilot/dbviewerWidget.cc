...
void GenericDBWidget::slotDBType(int mode)
{
    ...
    {
        case 1:
            dir.setNameFilter(CSL1("*.prc"));
            break;
        case 2:
            dir.setNameFilter(CSL1("*.pdb"));
            break;
        case 0:
        default:
...
void GenericDBWidget::slotDeleteRecord()
{
    ...
    if(currRecItem
            && (KMessageBox::questionYesNo(this, i18n("<qt>Do you really want to delete the selected record? This cannot be undone.<br><br>Delete record?<qt>"),
                                           i18n("Deleting Record"), KStdGuiItem::del(), KStdGuiItem::cancel()) == KMessageBox::Yes))
    {
        PilotRecord *rec = (PilotRecord *)currRecItem->rec();
        rec->setDeleted();
        writeRecord(rec);
