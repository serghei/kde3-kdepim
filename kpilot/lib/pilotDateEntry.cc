...
QString PilotDateEntry::getTextRepresentation(Qt::TextFormat richText)
{
    ...
    if(isAlarmEnabled())
    {
        ...
        {
            case advMinutes:
                tmp = tmp.arg(i18n("minutes"));
                break;
            case advHours:
                tmp = tmp.arg(i18n("hours"));
                break;
            case advDays:
                tmp = tmp.arg(i18n("days"));
                break;
            default:
                tmp = tmp.arg(QString::null);
                break;;
        }
        text += tmp;
        text += ps;
    ...
    if(getRepeatType() != repeatNone)
    {
        ...
        {
            case repeatDaily:
                tmp = tmp.arg(i18n("day(s)"));
                break;
            case repeatWeekly:
                tmp = tmp.arg(i18n("week(s)"));
                break;
            case repeatMonthlyByDay:
            case repeatMonthlyByDate:
                tmp = tmp.arg(i18n("month(s)"));
                break;
            case repeatYearly:
                tmp = tmp.arg(i18n("year(s)"));
                break;
            default:
                tmp = tmp.arg(QString::null);
                break;
