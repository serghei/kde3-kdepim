...
int DOCConverter::findBmkEndtags(QString &text, bmkList &fBmks)
{
    ...
    while(pos >= 0/* && doSearch*/)
    {
        ...
        if(pos < 0 || text[pos] != '>')
        {
            DEBUGKPILOT << "Current character \'" << text[pos].latin1() << "\' at position " << pos << " is not and ending >. Finish searching for bookmarks." <<
                        endl;
        ...
        else
        {
            ...
            while(doSearch && pos > 0)
            {
                ...
                if(text[pos] == '\n')
                {
                    DEBUGKPILOT << "Found carriage return at position " << pos << " inside the bookmark text, assuming this is not a bookmark, and the text ends in a >"
                                << endl;
                    doSearch = false;
                    pos = -1;
                    break;
