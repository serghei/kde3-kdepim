...
public:
    ...
    {
        return to< QValueList<int> >(mValue);
    }
    QValueList<unsigned int> uintValueList() const
    {
        return to< QValueList<unsigned int> >(mValue);
    }
    ...
    void setIntValueList(const QValueList<int> &l)
    {
        setValue(from(l));
    }
    void setUIntValueList(const QValueList<unsigned int> &l)
    {
        setValue(from(l));
    }
...
bool Kleo::ChiasmusBackend::checkForChiasmus(QString *reason) const
{
    ...
    assert(path);
    assert(path->argType() == CryptoConfigEntry::ArgType_Path);
