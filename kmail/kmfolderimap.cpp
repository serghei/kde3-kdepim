 ...
 *
 *  This file is based on kmacctimap.cpp by Michael Haeckel which is
...
KMFolderImap::KMFolderImap(KMFolder *folder, const char *name)
    : KMFolderMbox(folder, name)
    ...
    , mMailCheckProgressItem(nullptr)
    , mListDirProgressItem(nullptr)
    , mAddMessageProgressItem(nullptr)
    , mReadOnly(false)
    , mUploadAllFlags(false)
{
    mHasChildren = ChildrenUnknown;
    connect(this, SIGNAL(folderComplete(KMFolderImap *, bool)), this, SLOT(slotCompleteMailCheckProgress()));
}
...
{
    kdDebug(5006) << "KMFolderImap::reallyDoClose(owner: " << owner << ")" << endl;

    if(isSelected())
    {
        kdWarning(5006) << "    trying to close the selected folder " << label() << " - ignoring!" << endl;
        return;
    }
...
KMMessage *KMFolderImap::getMsg(int idx)
{
    ...
    // set it incomplete as the msg was not transferred from the server
    if(msg)
...
KMAcctImap *KMFolderImap::account() const
{
    if(!mAccount)
    {
        ...
        if(!parentFolderDir)
        {
            kdWarning() << "KMFolderImap::account() => no parent folder dir found for " << name() << endl;
            return nullptr;
        }
        ...
        if(!parentFolder)
        {
            kdWarning() << "KMFolderImap::account() => no parent folder found for " << name() << endl;
            return nullptr;
        }
...
void KMFolderImap::setAccount(KMAcctImap *account)
{
    mAccount = account;
    ...
    {
        if(!node->isDir())
            static_cast<KMFolderImap *>(static_cast<KMFolder *>(node)->storage())->setAccount(account);
    }
}
...
void KMFolderImap::remove()
{
    ...
    if(account()->makeConnection() == ImapAccountBase::Error || imapPath().isEmpty())
    {
    ...
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotRemoveFolderResult(KIO::Job *)));
}
...
int KMFolderImap::rename(const QString &newName, KMFolderDir *parent)
{
    if(!parent)
...
void KMFolderImap::addMsgQuiet(KMMessage *msg)
{
    ...
    if(parentFolder)
    {
        ...
        kmkernel->undoStack()->pushSingleAction(serNum, parentFolder, folder());
        int idx = parentFolder->find(msg);

        assert(idx != -1);
        parentFolder->take(idx);
    }
    else
    {
        kdDebug(5006) << "KMFolderImap::addMsgQuiet() => no parent" << endl;
    }
...
void KMFolderImap::addMsgQuiet(QPtrList<KMMessage> msgList)
{
    ...
    KMFolder *parentFolder = msgList.first()->parent();
    int undoId = -1;
    ...
    {
        if(undoId == -1)
            undoId = kmkernel->undoStack()->newUndoAction(parentFolder, folder());
        ...
        if(!uidplus)
        {
            // Remember the status with the MD5 as key
            // so it can be transfered to the new message
            mMetaDataMap.insert(msg->msgIdMD5(), new KMMsgMetaData(msg->status(), msg->getMsgSerNum()));
        }
...
int KMFolderImap::addMsg(KMMessage *msg, int *index_ret)
{
    ...
    index_ret = &index.first();
    return ret;
}


int KMFolderImap::addMsg(QPtrList<KMMessage> &msgList, QValueList<int> &index_ret)
{
    KMMessage *aMsg = msgList.getFirst();
    KMFolder *msgParent = aMsg->parent();
    ImapJob *imapJob = nullptr;
            ...
            if(static_cast<KMFolderImap *>(msgParent->storage())->account() == account())
            {
                ...
                if(folder() == msgParent)
                {
                    ...
                    for(KMMessage *msg = msgList.first(); msg; msg = msgList.next())
                    {
                        ...
                        connect(imapJob, SIGNAL(messageStored(KMMessage *)), SLOT(addMsgQuiet(KMMessage *)));
                        connect(imapJob, SIGNAL(result(KMail::FolderJob *)), SLOT(slotCopyMsgResult(KMail::FolderJob *)));
                ...
                else
                {
                    ...
                    for(QStringList::Iterator it = sets.begin(); it != sets.end(); ++it)
                    {
                        ...
                        if(temp_msgs.isEmpty())
                            kdDebug(5006) << "Wow! KMFolderImap::splitMessageList() returned an empty list!" << endl;

                        imapJob = new ImapJob(temp_msgs, *it, ImapJob::tMoveMessage, this);

                        connect(imapJob, SIGNAL(messageCopied(QPtrList<KMMessage>)), SLOT(addMsgQuiet(QPtrList<KMMessage>)));
                        connect(imapJob, SIGNAL(result(KMail::FolderJob *)), SLOT(slotCopyMsgResult(KMail::FolderJob *)));
...
{
    // getFolder() will not be called in this case
    if(job->error())
    {
        kdDebug(5006) << "KMFolderImap::slotCopyMsgResult() => " << job->error() << endl;
...
void KMFolderImap::copyMsg(QPtrList<KMMessage> &msgList)
{
    ...
    for(QStringList::Iterator it = sets.begin(); it != sets.end(); ++it)
    {
        ...
        connect(job, SIGNAL(result(KMail::FolderJob *)), SLOT(slotCopyMsgResult(KMail::FolderJob *)));
...
QPtrList<KMMessage> KMFolderImap::splitMessageList(const QString &set, QPtrList<KMMessage> &msgList)
{
    int lastcomma = set.findRev(",");
    int lastdub = set.findRev(":");
    int last = 0;

    if(lastdub > lastcomma)
        last = lastdub;
    else
        last = lastcomma;
...
{
    disconnect(account(), SIGNAL(connectionResult(int, const QString &)), this, SLOT(slotListNamespaces()));

    ImapAccountBase::ConnectionState connState = account()->makeConnection();

    if(connState == ImapAccountBase::Error)
    {
        kdWarning(5006) << "KMFolderImap::slotListNamespaces() => no connection, abort" << endl;
        return;
    }
    else if(connState == ImapAccountBase::Connecting)
    {
        kdDebug(5006) << "KMFolderImap::slotListNamespaces() => waiting for connection" << endl;

        // wait for the connectionResult
        connect(account(), SIGNAL(connectionResult(int, const QString &)), this, SLOT(slotListNamespaces()));
        return;
    }

    kdDebug(5006) << "KMFolderImap::slotListNamespaces()" << endl;
    ...
    for(QStringList::Iterator it = personal.begin(); it != personal.end(); ++it)
    {
        ...
        connect(job,
                SIGNAL(receivedFolders(const QStringList &, const QStringList &, const QStringList &, const QStringList &, const ImapAccountBase::jobData &)),
                SLOT(slotListResult(const QStringList &, const QStringList &, const QStringList &, const QStringList &, const ImapAccountBase::jobData &)));
    ...
    for(QStringList::Iterator it = ns.begin(); it != ns.end(); ++it)
    {
        ...
        connect(job,
                SIGNAL(receivedFolders(const QStringList &, const QStringList &, const QStringList &, const QStringList &, const ImapAccountBase::jobData &)),
                SLOT(slotCheckNamespace(const QStringList &, const QStringList &, const QStringList &, const QStringList &, const ImapAccountBase::jobData &)));
...
void KMFolderImap::slotCheckNamespace(
...
{
    kdDebug(5006) << "KMFolderImap::slotCheckNamespace(): " << subfolderNames.join(",") << endl;
    ...
    if(name.isEmpty())
    {
        ...
        slotListResult(subfolderNames, subfolderPaths, subfolderMimeTypes, subfolderAttributes, jobData);
        return;
    }

    folder()->createChildFolder();

    KMFolderNode *node = nullptr;
    for(node = folder()->child()->first(); node; node = folder()->child()->next())
    {
    ...
    else
    {
        if(node)
        {
            ...
            {
                KMFolderImap *nsFolder = static_cast<KMFolderImap *>(static_cast<KMFolder *>(node)->storage());
                nsFolder->slotListResult(subfolderNames, subfolderPaths, subfolderMimeTypes, subfolderAttributes, jobData);
            }
        }
        else
        {
            ...
            if(fld)
            {
                ...
                f->initializeFrom(this, account()->addPathToNamespace(name), "inode/directory");
                f->close("kmfolderimap_create");

                if(!account()->listOnlyOpenFolders())
                    f->slotListResult(subfolderNames, subfolderPaths, subfolderMimeTypes, subfolderAttributes, jobData);
            }
...
{
    if(!account() || (account() && account()->makeConnection() == ImapAccountBase::Error))
    {
        kdDebug(5006) << "KMFolderImap::listDirectory() - no connection, abort" << endl;
        return false;
    }
        ...
        const ImapAccountBase::jobData &jobData)
{
    ...
    if(root && !subfolderNames.empty())
    {
        ...
        if(parent)
        {
            kdDebug(5006) << "KMFolderImap::slotListResult() => pass listing to " << parent->label() << endl;
            parent->slotListResult(subfolderNames, subfolderPaths, subfolderMimeTypes, subfolderAttributes, jobData);
    ...
    for(uint i = 0; i < subfolderNames.count(); i++)
    {
        ...
        {
            kdDebug(5006) << "KMFolderImap::slotListResult() => create folder " << subfolderNames[i] << endl;
        ...
        if(f)
        {
            ...
            if(account()->listOnlyOpenFolders() && f->hasChildren() != FolderStorage::ChildrenUnknown)
                settingsChanged = true;
            ...
            if((subfolderMimeTypes[i] == "message/directory" || subfolderMimeTypes[i] == "inode/directory") && !account()->listOnlyOpenFolders())
                f->listDirectory();
        }
        else
        {
...
void KMFolderImap::initInbox()
{
    ...
    {
        f = static_cast<KMFolderImap *>(folder()->child()->createFolder("INBOX", true)->storage());
...
void KMFolderImap::checkFolders(const QStringList &subfolderNames, const QString &myNamespace)
{
    ...
    while(node)
    {
        if(!node->isDir() && subfolderNames.findIndex(node->name()) == -1)
        {
            ...
            bool isInNamespace = (myNamespace.isEmpty() || myNamespace == account()->namespaceForFolder(imapFld));

            kdDebug(5006) << node->name() << "KMFolderImap::checkFolders() => in namespace " << myNamespace << ":" << isInNamespace << endl;
            ...
            if(!ignore)
            {
                kdDebug(5006) << "KMFolderImap::checkFolders() => " << node->name() << " disappeared" << endl;
            ...
            {
                kdDebug(5006) << "KMFolderImap::checkFolders() => " << node->name() << " ignored" << endl;
            }
        }
...
void KMFolderImap::initializeFrom(KMFolderImap *parent, QString folderPath, QString mimeType)
{
...
void KMFolderImap::checkValidity()
{
    ...
    if(!account())
    {
        kdDebug(5006) << "    no account, abort" << endl;
    ...
    kdDebug(5006) << "     path: " << imapPath() << endl;

    // Start with a clean state
    disconnect(account(), SIGNAL(connectionResult(int, const QString &)), this, SLOT(checkValidity()));

    // Only check once at a time.
    if(mCheckingValidity)
    {
        kdDebug(5006) << "    already checking" << endl;
    ...
    {
        case ImapAccountBase::Error:
        {
            kdDebug(5006) << "    no connection, abort" << endl;
        ...
        case ImapAccountBase::Connecting:
        {
            kdDebug(5006) << "    waiting for connection" << endl;

            connect(account(), SIGNAL(connectionResult(int, const QString &)), this, SLOT(checkValidity()));
            return;
        }
    }
    ...
    KIO::Scheduler::assignJobToSlave(account()->slave(), job);
    account()->insertJob(job, jd);
    connect(job, SIGNAL(result(KIO::Job *)), SLOT(slotCheckValidityResult(KIO::Job *)));
    connect(job, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotSimpleData(KIO::Job *, const QByteArray &)));
...
{
    kdDebug(5006) << "KMFolderImap::slotCheckValidityResult() - " << fileName() << endl;
    ...
    else
    {
        ...
        {
            // uidValidity changed
            kdDebug(5006) << "KMFolderImap::slotCheckValidityResult() => uidValidty changed from "
...
void KMFolderImap::getAndCheckFolder(bool force)
{
    ...
    if(force)
        mCheckFlags = true; // force an update
}
...
void KMFolderImap::reallyGetFolder(const QString &startUid)
{
    ...
    if(startUid.isEmpty())
    {
        ...
        connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotListFolderResult(KIO::Job *)));
        connect(job, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), this, SLOT(slotListFolderEntries(KIO::Job *, const KIO::UDSEntryList &)));
    }
    else
    {
        ...
        connect(newJob, SIGNAL(result(KIO::Job *)), this, SLOT(slotGetLastMessagesResult(KIO::Job *)));
        connect(newJob, SIGNAL(data(KIO::Job *, const QByteArray &)), this, SLOT(slotGetMessagesData(KIO::Job *, const QByteArray &)));
    }
}
...
void KMFolderImap::slotListFolderResult(KIO::Job *job)
{
    ...
    if(job->error())
    {
        account()->handleJobError(job, i18n("Error while listing the contents of the folder %1.").arg(label()));
        account()->removeJob(it);
...
void KMFolderImap::slotListFolderEntries(KIO::Job *job, const KIO::UDSEntryList &uds)
{
    ImapAccountBase::JobIterator it = account()->findJob(job);
    ...
    long int flags = 0;
    for(KIO::UDSEntryList::ConstIterator udsIt = uds.begin(); udsIt != uds.end(); udsIt++)
    {
        for(KIO::UDSEntry::ConstIterator eIt = (*udsIt).begin(); eIt != (*udsIt).end(); eIt++)
        {
        ...
        if((mimeType == "message/rfc822-imap" || mimeType == "message/rfc822") && !(flags & 8))
        {
...
void KMFolderImap::flagsToStatus(KMMsgBase *msg, int flags, bool newMsg, int supportedFlags)
{
    ...
    {
        if(((supportedFlags & imapFlagMap[i].imapFlag) == 0 && (supportedFlags & 64) == 0) && !imapFlagMap[i].standardFlag)
            continue;
...
void KMFolderImap::ignoreJobsForMessage(KMMessage *msg)
{
    if(!msg
       || msg->transferInProgress()
       || !msg->parent()
       || msg->parent()->folderType() != KMFolderTypeImap)
        return;

    KMAcctImap *account = static_cast<KMFolderImap *>(msg->storage())->account();
...
void KMFolderImap::slotGetMessagesData(KIO::Job *job, const QByteArray &data)
{
    ...
    if(pos > 0)
    {
        ...
        {
            bool ok;
            int exists = (*it).cdata.mid(c + 10, (*it).cdata.find("\r\n", c + 1) - c - 10).toInt(&ok);
    ...
    while(pos >= 0)
    {
        ...
        if(pos != 14)
        {
            ...
            if(uid <= lastUid() && serNum > 0)
                ok = false; // the UID is already known so no need to create it
            ...
            {
                if(serNum > 0)
                    msg->setMsgSerNum(serNum); // assign the sernum from the cache
...
FolderJob *KMFolderImap::doCreateJob(
        KMMessage *msg,
...
{
    KMFolderImap *kmfi = folder ? dynamic_cast<KMFolderImap *>(folder->storage()) : nullptr;
...
void KMFolderImap::createFolder(const QString &name, const QString &parentPath, bool askUser)
{
    ...
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotCreateFolderResult(KIO::Job *)));
}
...
void KMFolderImap::slotCreateFolderResult(KIO::Job *job)
{
    ...
    if(job->error())
    {
        if(job->error() == KIO::ERR_COULD_NOT_MKDIR)
            account()->listDirectory(); // Creating a folder failed, remove it from the tree.
...
static QTextCodec *sUtf7Codec = nullptr;

QTextCodec *KMFolderImap::utf7Codec()
{
    if(!sUtf7Codec)
        sUtf7Codec = QTextCodec::codecForName("utf-7");
...
void KMFolderImap::deleteMessage(KMMessage *msg)
{
    ...
    connect(job, SIGNAL(result(KIO::Job *)), account(), SLOT(slotSimpleResult(KIO::Job *)));
}
...
void KMFolderImap::setStatus(QValueList<int> &_ids, KMMsgStatus status, bool toggle)
{
    ...
    if(mContentState == imapListingInProgress)
    {
        ...
        // create a new one
        kdDebug(5006) << "KMFolderImap::setStatus() => set status during folder listing, restarting listing." << endl;
...
void KMFolderImap::expungeFolder(KMFolderImap *folder, bool quiet)
{
    folder->setNeedsCompacting(false);
    KURL url = account()->getUrl();
    url.setPath(folder->imapPath() + ";UID=*");
...
{
    disconnect(account(), SIGNAL(connectionResult(int, const QString &)), this, SLOT(slotProcessNewMail(int, const QString &)));
...
bool KMFolderImap::processNewMail(bool)
{
    ...
    switch(connState)
    {
        ...
        case ImapAccountBase::Connecting:
        {
            ...
            connect(account(), SIGNAL(connectionResult(int, const QString &)), SLOT(slotProcessNewMail(int, const QString &)));
            return true;
        }
    }
...
void KMFolderImap::setUserRights(unsigned int userRights)
{
    ...
    kdDebug(5006) << "KMFolderImap::setUserRights(userRights: " << userRights << ") => imapPath(): " << imapPath() << endl;
}
...
{
    disconnect(account(), SIGNAL(connectionResult(int, const QString &)), this, SLOT(slotCreatePendingFolders(int, const QString &)));

    if(!errorCode)
    {
        for(const auto &folder : mFoldersPendingCreation)
            createFolder(folder);
    }
...
void KMFolderImap::search(const KMSearchPattern *pattern)
{
    ...
    connect(job, SIGNAL(searchDone(QValueList<Q_UINT32>, const KMSearchPattern *, bool)), this, SLOT(slotSearchDone(QValueList<Q_UINT32>, const KMSearchPattern *, bool)));
    job->start();
}


void KMFolderImap::slotSearchDone(
        QValueList<Q_UINT32> serNums,
...
void KMFolderImap::search(const KMSearchPattern *pattern, Q_UINT32 serNum)
{
    ...
    connect(job, SIGNAL(searchDone(Q_UINT32, const KMSearchPattern *, bool)), this, SLOT(slotSearchDone(Q_UINT32, const KMSearchPattern *, bool)));
    job->start();
}


void KMFolderImap::slotSearchDone(Q_UINT32 serNum, const KMSearchPattern *pattern, bool matches)
{
...
{
    return (hasChildren() == HasNoChildren && !folder()->isSystemFolder() ? true : false);
}
...
const ulong KMFolderImap::serNumForUID(ulong uid)
{
    ...
    kdDebug(5006) << "KMFolderImap::serNumForUID(uid: " << uid << "): unknown uid" << endl;
    return 0;
}
...
{
    if(path.isEmpty())
        kdWarning(5006) << "KMFolderImap::setImapPath() => ignoring empty path" << endl;
    else
        mImapPath = path;
}
