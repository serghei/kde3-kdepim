...
void KMFolderCachedImap::serverSync(bool recurse)
{
    if(mSyncState != SYNC_STATE_INITIAL)
    {
        if(KMessageBox::warningYesNo(0,
                                     i18n("Folder %1 is not in initial sync state (state was %2). Do you want to reset it to initial sync state and sync anyway?").arg(imapPath()).arg(
                                         mSyncState), QString::null, i18n("Reset && Sync"), KStdGuiItem::cancel()) == KMessageBox::Yes)
        {
        ...
        if(!node)
        {
            ...
            if(!locallyDeleted && mAccount->isPreviouslyDeletedFolder(subfolderPath))
            {
                locallyDeleted = KMessageBox::warningYesNo(
                                     0, i18n("<qt><p>It seems that the folder <b>%1</b> was deleted. Do you want to delete it from the server?</p></qt>").arg(mSubfolderNames[i]),
                                     QString::null, KStdGuiItem::del(), KStdGuiItem::cancel()) == KMessageBox::Yes;
            }
...
void KMFolderCachedImap::updateAnnotationFolderType()
{
    ...
    if(newType != oldType || newSubType != oldSubType)
    {
        ...
        kdDebug(5006) << mImapPath << ": updateAnnotationFolderType: '" << mAnnotationFolderType << "', was (" << oldType << " " << oldSubType <<
            ...
            for(uint i = 0 ; i <= ContentsTypeLast; ++i)
            {
                ...
                if(type == KMailICalIfaceImpl::annotationForContentsType(contentsType))
                {
                            ...
                            && subtype == "default")
                    {
                        ...
                        kdDebug(5006) << mImapPath << ": slotGetAnnotationResult: parent folder is " << folder()->parent()->owner()->idString() <<
    ...
    if(annjob->error())
    {
        if(job->error() == KIO::ERR_UNSUPPORTED_ACTION)
        {
            ...
            if(GlobalSettings::self()->theIMAPResourceStorageFormat() == GlobalSettings::EnumTheIMAPResourceStorageFormat::XML
                ...
                KMessageBox::error(0,
                                   i18n("The IMAP server %1 does not have support for IMAP annotations. The XML storage cannot be used on this server; please re-configure KMail differently.").arg(
                                       mAccount->host()));
    ...
    if(annjob->error())
    {
        if(job->error() == KIO::ERR_UNSUPPORTED_ACTION)
        {
            ...
            if(GlobalSettings::self()->theIMAPResourceStorageFormat() == GlobalSettings::EnumTheIMAPResourceStorageFormat::XML
                    && (uint)GlobalSettings::self()->theIMAPResourceAccount() == mAccount->id())
                KMessageBox::error(0,
                                   i18n("The IMAP server %1 doesn't have support for imap annotations. The XML storage cannot be used on this server, please re-configure KMail differently").arg(
                                       mAccount->host()));
            mAccount->setHasNoAnnotationSupport();
        }
        else
