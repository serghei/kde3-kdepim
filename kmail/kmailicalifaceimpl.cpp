...
static DwBodyPart *findBodyPartByMimeType(const KMMessage &msg, const char *sType, const char *sSubtype, bool startsWith = false)
{
    ...
    while(part)
    {
        ...
        if(part->hasHeaders())
        {
            ...
            else if(contentType.TypeStr() == sType
...
KMMessage *KMailICalIfaceImpl::findMessageBySerNum(Q_UINT32 serNum, KMFolder *folder)
{
    ...
    {
        kdWarning(5006) << "findMessageBySerNum( " << serNum << " ) found it in folder " << aFolder->location() << ", expected " << folder->location() <<
                        endl;
    }
    else
    {
                                        ...
                                        const QString &contentsType)
{
    ...
    StorageFormat defaultFormat = GlobalSettings::self()->theIMAPResourceStorageFormat() == GlobalSettings::EnumTheIMAPResourceStorageFormat::XML ?
                                  StorageXML : StorageIcalVcard;
