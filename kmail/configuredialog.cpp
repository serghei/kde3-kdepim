...
void AppearancePage::HeadersTab::save()
{
    ...
    // check bounds:
    assert(dateDisplayID >= 0);
    assert(dateDisplayID < numDateDisplayConfig);
        ...
        bool isList)
{
    ...
    {
        kdWarning(5006) << QString("Backend error: gpgconf has wrong type for %1/%2/%3: %4 %5").arg(componentName, groupName,
                        entryName).arg(entry->argType()).arg(entry->isList()) << endl;
        return 0;
    }
    return entry;
}
    ...
    : ConfigModuleTab(parent, name)
{
    ...
    QToolTip::add(mExchangeCompatibleInvitations,
                  i18n("Microsoft Outlook, when used in combination with a Microsoft Exchange server, has a problem understanding standards-compliant groupware e-mail. Turn this option on to send groupware invitations in a way that Microsoft Exchange understands."));
