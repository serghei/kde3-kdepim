...
static unsigned int IMAPRightsToPermission(const QString &str, const KURL &url, const QString &user)
{
    ...
    for(uint i = 0; i < len; ++i)
    {
        ...
        switch(ch.latin1())
        {
            ...
            case 'r':
                ...
                break;
            case 's':
                perm |= ACLJobs::WriteSeenFlag;
                break;
            case 'w':
                perm |= ACLJobs::WriteFlags;
                break;
            case 'i':
                perm |= ACLJobs::Insert;
                break;
    ...
    if((perm & ACLJobs::Read) && !(perm & ACLJobs::WriteSeenFlag))
    {
        ...
        kdWarning(5006) << "IMAPRightsToPermission: found read (r) but not seen (s). Things will not work well for folder " << url << " and user " <<
                        (user.isEmpty() ? "myself" : user) << endl;
...
void ACLJobs::GetACLJob::slotInfoMessage(KIO::Job *, const QString &str)
{
    ...
    {
        QString user = lst.front();
        lst.pop_front();
        QString imapRights = lst.front();
        lst.pop_front();
