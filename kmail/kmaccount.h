 ...
 *
\************************************************************************/
#ifndef _KMACCOUNT_H_
#define _KMACCOUNT_H_
...
public:
    KMPrecommand(const QString &precommand, QObject *parent = nullptr);
...
public:
     ...
     */
    virtual void pseudoAssign(const KMAccount *account);

    /**
     * There can be exactly one folder that is fed by messages from an
     * account. */
    KMFolder *folder(void) const { return reinterpret_cast<KMFolder*>(static_cast<KMAcctFolder*>(mFolder)); }
    virtual void setFolder(KMFolder *folder, bool addAccount = false);

    /**
     * the id of the trash folder (if any) for this account
     */
    QString trash() const { return mTrash; }
    virtual void setTrash(const QString &name) { mTrash = name; }
     ...
     */
    virtual void setCheckInterval(int newInterval);
    int checkInterval() const;

    /**
     * This can be used to provide a more complex calculation later if we want
     */
    inline int defaultCheckInterval() const { return DefaultCheckInterval; }
    ...
    static QString importPassword(const QString &str);
     ...
     * indicators.
     */
    ProgressItem *mailCheckProgressItem() const { return mMailCheckProgressItem; }
