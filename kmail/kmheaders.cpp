...
bool KMHeaders::eventFilter(QObject *o, QEvent *e)
{
            ...
            o->isA("QHeader"))
    {
        ...
        else if(mFolder && (mFolder->whoField().lower() == "to"))
    ...
    if(sortStream)
    {
        ...
        if(version == KMAIL_SORT_VERSION)
        {
            ...
            if(byteOrder == 0x12345678)
            {
                ...
                {
                    kdDebug(5006) << endl << "Whoa: x " << x << ", sorted_count " << sorted_count << ", discovered_count " << discovered_count << ", count " <<
                                  mFolder->count() << endl << endl;
                    fclose(sortStream);
                    sortStream = 0;
                }
