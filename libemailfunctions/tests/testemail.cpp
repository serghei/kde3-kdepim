...
static bool checkGetNameAndEmail(const QString &input, const QString &expName, const QString &expEmail, bool expRetVal)
{
    ...
    check("getNameAndMail " + input + " retVal", retVal ? QString::fromLatin1("true") : QString::fromLatin1("false"),
...
int main(int argc, char *argv[])
{
    ...
    checkSplitEmailAddrList("Matt Douhan <matt@fruitsalad.org>, Foo Bar <foo@bar.com>",
                            QStringList() << "Matt Douhan <matt@fruitsalad.org>" << "Foo Bar <foo@bar.com>");
    checkSplitEmailAddrList("\"Matt, Douhan\" <matt@fruitsalad.org>, Foo Bar <foo@bar.com>",
                            QStringList() << "\"Matt, Douhan\" <matt@fruitsalad.org>" << "Foo Bar <foo@bar.com>");
    ...
    checkNormalizeAddressesAndDecodeIDNs("=?iso-8859-1?B?5Hf8b2xmLPZBbmRyZWFz?= <nobody@example.org>",
